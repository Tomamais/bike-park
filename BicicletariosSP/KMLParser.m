...
- (NSArray *)points
{
    ...
    for (KMLPlacemark *placemark in _placemarks) {
        ...
        if (point) {
            MapPoint *placeObject = [[MapPoint alloc] initWithName:[placemark name] address:[placemark placemarkDescription] coordinate:[point coordinate]];
            [points addObject:placeObject];
        }
    }
    return points;
}

- (NSArray *)sortPoints:(CLLocationCoordinate2D)coordinate points:(NSArray *)points
{
    // firts, fill the distances
    for (MapPoint *point in points) {
        CLLocation *locA = [[CLLocation alloc] initWithLatitude:point.coordinate.latitude longitude:point.coordinate.longitude];
        CLLocation *locB =[[CLLocation alloc] initWithLatitude:coordinate.latitude longitude:coordinate.longitude];
        point.distance = [locA distanceFromLocation:locB];

    }
    
    NSSortDescriptor *sortDescriptor = [[NSSortDescriptor alloc] initWithKey:@"distance" ascending:YES];
    NSArray *sortDescriptors = [NSArray arrayWithObject:sortDescriptor];
    NSArray *sortedArray = [points sortedArrayUsingDescriptors:sortDescriptors];
